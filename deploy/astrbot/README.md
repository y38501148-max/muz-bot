...
引用与合并转发按 OneBot 消息段读取，不依赖只包含 `Plain` 段的纯文本
字段。引用消息通过 `get_msg` 补取，合并转发通过 `get_forward_msg` 展开；
其中的文字、图片、视频和文件可随本轮请求分析。引用资料作为不可信的
本轮用户内容传入，不进入 system prompt；每次最多展开 2 条转发、40 个
节点、200 个消息段和 8,000 字。为避免普通被动聊天意外上传整段记录，
只有 @/引用机器人或明确要求查看、分析、总结引用/转发时才会展开。
